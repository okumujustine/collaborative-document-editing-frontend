...
export default function GoogleAuth() {
  ...
  const googleSuccess = async (res) => {
    ...
    await ApiInstance.post('/auth', { user: result }).then(res => {

      try {
        dispatch({ type: AUTH, data: { result: res.data.user, token } });
        if (pathName) {
  ...
  return (
    <div className="flex justify-center items-center">
      <GoogleLogin
        clientId={clientId}
        render={(renderProps) => (
          <button className="text-blue-800 border-2 border-blue-800 mt-3 px-3 py-2 text-white hover:bg-blue-800 hover:text-white flex items-center focus:outline-none" onClick={renderProps.onClick} disabled={renderProps.disabled}>
